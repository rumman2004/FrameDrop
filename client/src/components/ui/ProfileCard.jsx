...
// ── Constants ──────────────────────────────────────────────────────────────
const DEFAULT_INNER_GRADIENT = 'linear-gradient(145deg, #1a0a2e99 0%, #0d1f3c88 50%, #0a0a1a66 100%)';
...
const ProfileCardComponent = ({
    ...
    const handleClick = () => {
      ...
      if (ME && typeof ME.requestPermission === 'function') {
        ME.requestPermission()
          .then((s) => { if (s === 'granted') window.addEventListener('deviceorientation', handleDeviceOrientation); })
          .catch(console.error);
      } else {
  ...
  const hasMiniAvatar = Boolean(miniAvatarUrl || avatarUrl);
      ...
      <div ref={shellRef} style={{ position: 'relative', zIndex: 1 }}>
        <section
          style={{
            ...
            // ── NO background here — let avatar show through fully ───
        ...
        >
          ...
          {/* ── Layer 0: Avatar (raw, no blend mode) ──────────────── */}
          <div
            style={{
              position:     'absolute',
              inset:        0,
            ...
            {hasAvatar ? (
              <img
                ...
                style={{
                  ...
                  display:         'block',
                  // ── KEY: no mixBlendMode, no filter — raw image ──
                  mixBlendMode:    'normal',
                  filter:          'none',
                  transform: `
          ...
          {/* ── Layer 1: Subtle dark vignette at edges ─────────────── */}
          <div
            aria-hidden="true"
            style={{
              position:     'absolute',
          ...
          {/* ── Layer 2: Top gradient so name text is readable ─────── */}
          <div
            aria-hidden="true"
            style={{
              position:     'absolute',
          ...
          {/* ── Layer 3: Bottom gradient so info bar is readable ────── */}
          <div
            aria-hidden="true"
            style={{
              position:     'absolute',
          ...
          {/* ── Layer 4: Glass shine — sweeping highlight ───────────── */}
          <div
            aria-hidden="true"
            style={{
              position:     'absolute',
          ...
          >
            ...
            {/* Sweeping shine streak — always animating */}
            <div
              style={{
                position:       'absolute',
                top:            '-20%',
            ...
            {/* Edge rim highlight — subtle border glow */}
            <div
              style={{
                position:     'absolute',
                inset:        0,
          ...
          >
            <h3
              style={{
                ...
                // ── soft text shadow for legibility on any photo ───
